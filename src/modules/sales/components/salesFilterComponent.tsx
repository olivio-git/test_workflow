...
const SalesFiltersComponent: React.FC<SalesFiltersProps> = ({
    ...
    useEffect(() => {
        if (!filters.codigo_oem_producto && !filters.codigo_interno && !filters.cliente) {
            setCodigoOEM("")
    ...
    return (
        <section className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                ...
                <div className="space-y-2">
                    <Label className="text-gray-700 text-sm font-medium">Buscar ID Cliente</Label>
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                        <Input
                            placeholder="Ej: 73"
                            value={clientId}
                            onChange={(e) => setClientId(e.target.value ? parseInt(e.target.value, 10) : undefined)}
                            className="pl-10 font-mono text-xs"
                        />
                    </div>
                </div>
