...
const originDialogConfig: DialogConfig = {
    title: "Procedencia",
...
const OriginsScreen = () => {
    ...
    const handleDeleteError = (error: unknown, id: number) => {
    ...
    const handleEditOrigin = (id: number) => {
        setEditingId(id);
        setIsDialogOpen(true);
    };
    ...
    const handleCreateSubmit = createForm.handleSubmit(async (data: CreateOrigin) => {
    ...
    const handleUpdateSubmit = updateForm.handleSubmit(async (data: UpdateOrigin) => {
        if (editingId) {
                    ...
                    <section className="grid gap-2 sm:grid-cols-2">
                        ...
                        <div className="flex gap-2 justify-end w-full">
                            <TooltipButton
                                ...
                                buttonProps={{
                                    className: 'w-8',
                                    disabled: isRefetchingOriginsData || isFetchingOriginsData,
                                }}
                                tooltip={"Recargar datos"}
                            >
                                <RefreshCcw className={`size-4 ${isRefetchingOriginsData || isFetchingOriginsData ? 'animate-spin' : ''}`} />
                            </TooltipButton>

                            <Select
                                value={filters.pagina_registros.toString()}
                                onValueChange={(value) => updateFilter('pagina_registros', parseInt(value))}
                            >
                                <SelectTrigger className="space-x-2 max-w-max">
                                    <SelectValue />
                                </SelectTrigger>
                ...
                <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                    <div>
                        ...
                        <CardDescription className="text-sm">
                            {originsData?.meta?.total || 0} elemento{(originsData?.meta?.total || 0) !== 1 ? "s" : ""} registrado
                            {(originsData?.meta?.total || 0) !== 1 ? "s" : ""}
                        </CardDescription>
                    </div>
                    <ConfigFormDialog
                        config={originDialogConfig}
                        isOpen={isDialogOpen}
